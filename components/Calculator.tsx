 ...
 const calculate = (): CalcResult => {
   ...
   try {
     ...
     if (calcData.slug?.toLowerCase().includes('mortgage-payment-affordability')) {
       ...
       if (principal <= 0) return 'Down payment too high check interest rate or amortization';
       }

       if (!isFinite(monthlyPayment) || isNaN(monthlyPayment)) {
         return 'Invalid mortgage calculation property tax or other values too high';
